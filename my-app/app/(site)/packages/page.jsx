...
export default function PackagesPage() {
  ...
  useEffect(() => {
    PackageService.getActive()
      ...
      .catch(err => {
        console.error("Lỗi gọi API packages:", err);
        setFallbackData();
      })
  ...
  return (
    <>
      <header className="page-header">
        <Image src="https://images.unsplash.com/photo-1469371670807-013ccf25f16a?q=80&w=2070&auto=format&fit=crop" alt="Packages Header" fill sizes="100vw" priority className="page-header-bg" />
      ...
      <div className="container section" style={{ maxWidth: "1200px" }}>
        {loading ? (
          <div style={{ textAlign: "center", padding: "5rem 0" }}>
            <p className="text-gradient-gold" style={{ fontSize: "1.5rem", letterSpacing: "2px" }}>ĐANG TẢI GÓI DỊCH VỤ...</p>
          </div>
        ) : (
            ...
            {packages.map((pkg, index) => (
              <div key={pkg.id} className="animate-fade-in-up" style={{ 
                ...
                <div style={{ flex: "1 1 50%", position: "relative", aspectRatio: "4/3", overflow: "hidden" }}>
                  <Image src={pkg.img} alt={pkg.name} fill sizes="(max-width: 768px) 100vw, 33vw" style={{ objectFit: "cover" }} />
                </div>
