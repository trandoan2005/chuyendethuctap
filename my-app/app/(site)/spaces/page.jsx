...
export default function SpacesPage() {
  ...
  useEffect(() => {
    TableService.getAll()
      ...
      .catch(err => {
        console.error("Lỗi gọi API tables:", err);
        setFallbackData();
      })
  ...
  return (
    <>
      <header className="page-header">
        <Image src="https://images.unsplash.com/photo-1537047902294-62a40c20a6ae?q=80&w=1974&auto=format&fit=crop" alt="Spaces Header" fill sizes="100vw" priority className="page-header-bg" />
      ...
      <div className="container section" style={{ maxWidth: "1200px" }}>
        {loading ? (
          <div style={{ textAlign: "center", padding: "5rem 0" }}>
            <p className="text-gradient-gold" style={{ fontSize: "1.5rem", letterSpacing: "2px" }}>ĐANG TẢI KHÔNG GIAN...</p>
          </div>
        ) : (
            ...
            {spaces.map((space, index) => (
              <div key={space.id} className="animate-fade-in-up" style={{ 
                ...
                <div style={{ flex: "1 1 50%", position: "relative", aspectRatio: "4/3", overflow: "hidden" }}>
                  <Image src={space.img} alt={space.name} fill sizes="(max-width: 768px) 100vw, 50vw" style={{ objectFit: "cover" }} />
                  <div style={{ 
